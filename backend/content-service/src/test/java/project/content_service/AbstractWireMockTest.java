...
public abstract class AbstractWireMockTest extends AbstractTest {
    ...
    @SneakyThrows
    protected <T> void stubSuccess(String url, HttpMethod method, Map<String, String> headers, T data) {
        BaseResponse<T> response = BaseResponse.<T>builder()
                .data(data)
                .message("ok")
                .build();

        var mapping = request(method.name(), urlEqualTo(url));
        headers.forEach((name, value) -> mapping.withHeader(name, equalTo(value)));

        stubFor(mapping.willReturn(okJson(objectMapper.writeValueAsString(response))));
    }
